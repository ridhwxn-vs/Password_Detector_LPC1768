...
void LEDFail(void)                          // LED Must Blink Parallely together for showing Unsuccessful Password Attempt
{
	LPC_PINCON->PINSEL0 &= 0xFF0000FF;      // GPIO and Output Initialization
	LPC_GPIO0->FIODIR |= 0x00000FF0;
	while(1)
	{
		LPC_GPIO0->FIOPIN = 0xFF0;          // Set all LEDs ON
		for(j=0;j<100000;j++);
		LPC_GPIO0->FIOPIN = 0x000;          // Set all LEDs OFF
		for(j=0;j<100000;j++);
		LPC_GPIO0->FIOPIN = 0xFF0;          // Set all LEDs ON
  }
}
...
int Main(void)                              
{
    int inputPassword[4];                   // For getting the Input from User
        ...
        while (passwordIndex < 4) 
        {
            ...
            if (flag == 1) 
            {
                for (i = 0; i < 16; i++)            // Get Corresponding Letter / Digit to Display
                {
                    ...
                    {
                        key = ASCII_CODE[i];
                        inputPassword[passwordIndex++] = key - 48;      // Input also Updated
                        disp[0] = key;
                        disp[1] = '\0';
                        disp[2] = '\0';
                        LCD_Puts(&disp[0]);                             // Displaying Pressed Key
                        Delay_LCD(200000);
                        break;
                    }
                }
